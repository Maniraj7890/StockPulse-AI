...
const AIExplanationCard = lazy(() => import('@/components/AIExplanationCard'));
const WhySignalPanel = lazy(() => import('@/components/WhySignalPanel'));

const DEFERRED_RENDER_DELAY_MS = 80;

function buildCandidateFallback(item) {
  return {
    summary:
      item.direction === 'UP'
        ? `${item.symbol} has a cleaner 1-hour upside setup based on the current rule engine.`
        : item.direction === 'DOWN'
          ? `${item.symbol} has a cleaner 1-hour downside setup based on the current rule engine.`
          : `${item.symbol} has mixed 1-hour conditions, so no clear edge is preferred.`,
    riskNote:
      item.marketStatus === 'CLOSED'
        ? 'Market is closed, so this view is based on last session data.'
        : `Risk level is ${item.riskLevel}, so the setup still needs confirmation.`,
    invalidationNote:
      item.invalidation > 0
        ? `The setup weakens if price moves through ${formatCurrency(item.invalidation)}.`
        : 'Use the engine invalidation level before acting.',
    actionNote:
      item.direction === 'UP'
        ? 'Favor upside only while momentum and support structure stay intact.'
        : item.direction === 'DOWN'
          ? 'Favor downside only while weakness and rejection structure stay intact.'
          : 'No-trade is preferred until indicators align more clearly.',
  };
}
        ...
        {showAI ? (
          <Suspense fallback={<DeferredSectionFallback label="Loading AI explanation..." />}>
            <AIExplanationCard
              ...
              eyebrow="AI Candidate View"
              payload={item}
              fallbackExplanation={buildCandidateFallback(item)}
            />
          </Suspense>
        ) : null}
      </div>
